    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          ...
          <el-tab-pane :label="'进行中(' + (arr[10] ? arr[10].total : 0) + ')'" name="10" />
          <el-tab-pane :label="'已拒绝(' + (arr[30] ? arr[30].total : 0) + ')'" name="30" />
          <el-tab-pane :label="'已同意(' + (arr[20] ? arr[20].total : 0) + ')'" name="20" />
          <el-tab-pane :label="'已完成(' + (arr[40] ? arr[40] : 0) + ')'" name="40" />
        ...
        <el-table v-loading="loading" :data="tableData" border style="width: 100%">
          ...
          <el-table-column prop="" label="处理状态" width="130">
            <template slot-scope="scope">
              ...
              <p>
                平台审核：
                <span v-if="scope.row.plate_status.value === 10" class="orange">{{ scope.row.plate_status.text }}</span>
                <span v-if="scope.row.plate_status.value === 20" class="orange">{{ scope.row.plate_status.text }}</span>
                <span v-if="scope.row.plate_status.value === 30" class="orange">{{ scope.row.plate_status.text }}</span>
              </p>
              <!-- 发货状态-->
              ...
              <!-- 商家收货状态-->
              <p v-if="scope.row.type.value === 10 && scope.row.plate_status.value === 20 && scope.row.is_user_send === 1 && scope.row.is_receipt === 0">
                <span>商家收货：</span>
          ...
          <el-table-column fixed="right" label="操作" width="80">
            <template slot-scope="scope">
              ...
              <p><el-button v-if="scope.row.plate_status.value === 10 && scope.row.status.value !== 30" type="text" size="small" @click="goDetail(scope.row.order_refund_id)">去审核</el-button></p>
            </template>
          </el-table-column>
        </el-table>
      </div>
