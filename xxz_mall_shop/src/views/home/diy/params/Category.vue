  ...
  <div>
    ...
    <el-form size="small" :model="curItem" label-width="100px">
          ...
          <div class="choice-category-list">
            <draggable v-model="curItem.data" :options="{draggable:'.item',animation:500}">
              <transition-group class="d-s-c f-w">
                <div v-for="(cate, index) in curItem.data" :key="cate.category_id" class="item">
      ...
      <el-form-item label="显示内容：">
        ...
        <el-checkbox v-show="curItem.style.column === 1" v-model="sellingPointShow" @change="checked => check(checked, 'sellingPoint')">商品卖点</el-checkbox>
        <el-checkbox v-show="curItem.style.column === 1" v-model="productSalesShow" @change="checked => check(checked, 'productSales')">商品销量</el-checkbox>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getData() {
      const self = this
      GoodsApi.showCatList({ page_id: self.page_id }, true)
        .then(res => {
    ...
    currCategoryAuto(list) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        for (let j = 0; j < this.curItem.data.length; j++) {
          ...
          if (this.curItem.data[j].hasOwnProperty('category_id') && item.category_id === this.curItem.data[j].category_id) {
    ...
    changeCategory() {
      ...
      for (let i = 0; i < this.currCategory.length; i++) {
        for (let j = 0; j < this.categoryList.length; j++) {
          if (this.currCategory[i] === this.categoryList[j].category_id) {
            this.curItem.data.push({ category_id: this.categoryList[j].category_id, name: this.categoryList[j].name })
          }
        }
      }
    },
