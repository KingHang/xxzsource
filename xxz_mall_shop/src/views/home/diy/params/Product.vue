    ...
    <el-form size="small" :model="curItem" label-width="100px">
      ...
      <template v-if="curItem.params.source === 'auto'">
        ...
        <el-form-item label="显示数量：">
          <el-input v-model="curItem.params.auto.showNum" class="w-auto" />
        </el-form-item>
      </template>
          ...
          <div class="choice-product-list">
            <draggable v-model="curItem.data" :options="{draggable:'.item',animation:500}">
              <transition-group class="d-s-c f-w">
                <div v-for="(product, index) in curItem.data" :key="product.product_id" class="item">
                  <div class="delete-box"><i class="el-icon-error" @click="$parent.onEditorDeleleData(index, selectedIndex)" /></div>
...
export default {
  ...
  methods: {
    ...
    getData() {
      const self = this
      GoodsApi.catList({ page_id: self.page_id }, true)
        .then(res => {
