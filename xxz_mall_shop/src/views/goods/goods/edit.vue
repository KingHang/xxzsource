<template>
  <div v-if="!loading" class="product-add">
    ...
    <el-form ref="form" size="small" :model="form" label-width="180px">
      ...
      <div class="common-button-wrapper" :style="old_audit === 0 || old_audit === 20 ? 'height: 100px' : 'height: 50px'">
        <Audit v-if="old_audit === 0 || old_audit === 20" />
        <el-button size="small" type="info" @click="cancelFunc">取消</el-button>
        <el-button v-if="old_audit === 10" size="small" type="primary" :disabled="save_loading" @click="onSubmit('edit')">发布</el-button>
        <el-button v-else size="small" type="primary" :disabled="save_loading" @click="onSubmit('audit')">审核</el-button>
      </div>
    </el-form>
  </div>
</template>
    ...
    getData() {
      ...
      GoodsApi.getEditData({
        ...
        if (res.data.model.alone_grade_equity != null && typeof res.data.model.alone_grade_equity !== 'undefined') {
          const equitys = res.data.model.alone_grade_equity
        ...
        self.form.model.params_value = res.data.model.category_params_value
        // 等级id转换成数组
        ...
        // 区别实物的自提和计次商品
        self.form.model.store_ids_one = res.data.model.product_type === 1 ? res.data.model.store_ids : ''
        self.form.model.storeList_one = res.data.model.product_type === 1 ? res.data.model.storeList : []
        self.form.model.store_ids_two = res.data.model.product_type === 3 || res.data.model.product_type === 4 ? res.data.model.store_ids : ''
        self.form.model.storeList_two = res.data.model.product_type === 3 || res.data.model.product_type === 4 ? res.data.model.storeList : []
        if (res.data.model.product_type === 1 && res.data.model.store_ids !== '') {
          self.$set(self.form.model, 'is_on_shelf_store_one', 1)
        } else {
          self.$set(self.form.model, 'is_on_shelf_store_one', 0)
        }
        if ((res.data.model.product_type === 3 || res.data.model.product_type === 4) && res.data.model.store_ids !== '') {
          self.$set(self.form.model, 'is_on_shelf_store_two', 1)
        } else {
          self.$set(self.form.model, 'is_on_shelf_store_two', 0)
        }
      }).catch(() => {
        self.loading = false
      })
