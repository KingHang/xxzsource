<template>
  <div class="d-s-s">
    ...
    <div class="model-container">
      <div class="img-box">
        <el-image v-if="formData.category_style === 10" :src="category_10" fit="fill" />
        <el-image v-if="formData.category_style === 20" :src="category_20" fit="fill" />
        <el-image v-if="formData.category_style === 30" :src="category_30" fit="fill" />
        <el-image v-if="formData.category_style === 40" :src="category_40" fit="fill" />
      </div>
    </div>
      ...
      <el-form size="small" :model="formData" label-width="100px">
        <el-form-item label="分类页样式：">
          <el-radio-group v-model="formData.category_style">
            ...
            <el-radio :label="40">一级分类(带商品)</el-radio>
          </el-radio-group>
...
export default {
  ...
  methods: {
    ...
    getData() {
      const self = this
      HomeApi.getCategory({}, true).then(res => {
    ...
    submitData() {
      const self = this
      self.loading = true
      const Params = self.formData
      HomeApi.postCategory(Params, true)
        .then(data => {
          self.loading = false
          self.$message({
            message: '恭喜你，修改成功',
...
<style scoped="scoped">
.model-container {width: 300px;height: calc(100vh - 150px);margin-right: 30px;}
.model-container img{width: 100%;}
.model-container .img-box{ box-shadow: 0 0 16px 0 rgba(0,0,0,.1);}
.param-container {padding: 20px;height: calc(100vh - 150px);border: 1px solid #cccccc;}
</style>
