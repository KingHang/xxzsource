<template>
  <div v-if="!loading" class="user pb50">
    <div class="product-content">
      ...
      <div v-if="order.supplier" class="refund-detail-content">
        <el-row>
          <el-col :span="5">
            <div class="pb16">
      ...
      <div class="apply-reason-box">
        ...
        <div v-if="detail.image.length > 0" class="d-s-s mt10">
          <div v-for="(item, index) in detail.image" :key="index" class="pic">
            <img v-img-url="item.file_path" alt="" width="60">
          </div>
        </div>
      </div>

      <!--商家审核-->
      <div v-if="detail.is_agree.value === 0 && detail.status.value !== 30">
        ...
        <el-form ref="form" size="small" :model="form" label-width="80px">
          ...
          <el-form-item v-if="form.is_agree === 10 && detail.type.value !== 30" label="退货地址">
          ...
          <el-form-item v-if="form.is_agree === 10" label="退款金额">
            <el-input v-model="form.refund_money" />
            <p>请输入退款金额，最多{{ detail.orderproduct.total_pay_price }} 元。注：该操作将执行订单原路退款 并关闭当前售后单，请确认并填写退款的金额（不能大于订单实付款）</p>
          </el-form-item>
          <el-form-item v-if="form.is_agree === 20" label="拒绝原因">
            <el-input v-model="form.refuse_desc" type="textarea" class="max-w460" />
          </el-form-item>
        </el-form>
      </div>
      ...
      <div v-if="detail.type.value === 10 && detail.is_agree.value === 10 && detail.is_user_send === 1 && detail.is_receipt === 0">
        ...
        <el-form ref="money" size="small" :model="money" label-width="80px">
          <p>注：该操作将执行订单原路退款 并关闭当前售后单，请确认并填写退款的金额（不能大于订单实付款）</p>
          <el-form-item label="售后类型">
            <span class="orange">{{ detail.type.text }}</span>
          </el-form-item>
          <el-form-item label="退款金额">
            <el-input v-model="money.refund_money" />
    ...
    <div class="common-button-wrapper">
      ...
      <!--商家审核-->
      <template v-if="detail.is_agree.value === 0 && detail.status.value !== 30">
        <el-button size="small" type="primary" @click="onSubmit(detail.order_refund_id)">确认审核</el-button>
      </template>
      <template v-if="detail.type.value === 10 && detail.is_agree.value === 10 && detail.is_user_send === 1 && detail.is_receipt === 0 && detail.status.value !== 30">
...
export default {
  ...
  methods: {
    ...
    getData() {
      const self = this
      const order_refund_id = this.$route.query.order_refund_id
      OrderApi.refundDetail(
        ...
        .then(res => {
          ...
          self.form.refund_money = self.detail.orderproduct.total_pay_price
          self.money.refund_money = self.detail.orderproduct.total_pay_price
