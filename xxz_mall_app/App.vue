			...
			getTabBarLinks(){
				uni.request({
				    ...
				    success: (result) => {
						...
						let vars = result.data.data;
						uni.setTabBarStyle({
							color: vars.bottomnav.no_color,
						    selectedColor: vars.bottomnav.color,
						})
						vars.nav.data.list.forEach((item,index)=>{
							if(index == 5){
								return;
							}
							uni.setTabBarItem({
						...
						// #ifdef  MP-WEIXIN
						if(vars.bottomnav.menu_type == 2){
							let tabbar = vars.nav.data.list[5];
							uni.setTabBarItem({
							  index:2,
							  text: tabbar.text,
						...
						uni.setStorageSync('middle', vars.bottomnav.menu_type);
						// #endif
						// #ifndef  MP-WEIXIN
