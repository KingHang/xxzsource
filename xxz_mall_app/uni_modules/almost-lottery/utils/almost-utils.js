...
const getLocalFilePath = (path) => {
  ...
  if (path.indexOf('/') === 0) {
		// ios 无法获取本地路径
    let localFilePath = plugin.os.name === 'iOS' ? path : plugin.io.convertLocalFileSystemURL(path)
	...
	return new Promise((resolve, reject) => {
		...
		if (typeof plus === 'object') {
			...
			plus.io.resolveLocalFileSystemURL(tempPath, (entry) => {
				entry.file((file) => {
					let fileReader = new plugin.io.FileReader()
...
export const base64ToPath = (base64) => {
	return new Promise((resolve, reject) => {
		...
		if (typeof plus === 'object') {
			let bitmap = new plugin.nativeObj.Bitmap('bitmap' + Date.now())
