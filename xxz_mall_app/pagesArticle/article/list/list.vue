		...
		<template>
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50" @scrolltolower="scrolltolowerFunc">
				<view class="article-list">
					<view class="item" v-for="(item, index) in listData" :key="index" @click="gotoDetail(item.news_id)">
						<view class="info">
...
export default {
	...
	methods: {
		...
		getCategory() {
			let self = this;
			self._get('plugin.news.article/category', {}, function(res) {
		...
		/*跳转文章详情*/
		gotoDetail(e) {
			this.gotoPage('/pagesArticle/article/detail/detail?article_id=' + e);
		}
	}
};
</script>
