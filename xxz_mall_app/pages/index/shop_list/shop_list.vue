		...
		<view class="prodcut-list-wrap">
			<scroll-view scroll-y="true" class="scroll-Y" :style="'height:' + scrollviewHigh + 'px;'" lower-threshold="50"
			 ...
			 @refresherrestore="onRestore">
                ...
                <view class="hlbblock" @click="goto_shop(item.shop_supplier_id)"  style="padding: 20rpx;width: 95%;margin: 15rpx" v-for="(item,index) in shopData">
                    <view style="" class="fsc">
                        <view style="display: flex;align-items: center">
                            <view style="width: 140rpx;height: 140rpx;">
                                <image :src="item.logos||'/static/images/shop/shopdefault.png'" style="width: 140rpx;height:140rpx;border-radius: 15rpx" mode="widthFix"></image>
                                <!--<u-avatar size="100" mode="square" :src="shop_info.logos"></u-avatar>-->

                            </view>
                            <view style="margin-left: 15rpx;margin-top: 20rpx">
                                <view style="font-weight: bold">
                                    {{item.name}}
                                </view>
                                <view style="font-size: 24rpx;color: #999999;padding: 10rpx 0">
                                    主营：{{item.category_name?item.category_name:'无'}} / 关注：{{item.fav_count}}人
                                </view>

                                <view style="display: flex">
                                    <view style="display: flex;background:#E9E9E9;color: #6E6564;font-size: 20rpx;padding: 5rpx 15rpx;border-radius: 20rpx;align-items: center;height: 32rpx">
                                        <view style="font-size: 20rpx;">
                                            店铺评分
                                        </view>
                                        <view style="margin-bottom: 8rpx;margin-left: 3rpx">
                                            <u-rate active-color="#F63E36"  gutter="5" size="20" v-model="item.server_score"   ></u-rate>
                                        </view>
                                    </view>
                                    <view style="font-size: 20rpx;color:#999999;margin-left: 3rpx">
                                        销量{{item.product_sales||0}}
                                    </view>
                                </view>
                        ...
                        <view>
                            ...
                            <block v-if="!item.isfollow">
                                <view @click.stop="guanzhu(item,index)">
                                    <view class="hlbbutton"  style="width: 130rpx;display: flex;font-size: 24rpx;padding:5rpx 10rpx;border-radius: 50rpx;align-items: center;justify-content: center">
                                        <u-icon name="plus" color="white" style="margin-right: 0rpx"></u-icon>
                                        <view style="font-weight: bold">
                                            关注
                                        </view>
                                    </view>
                            ...
                            <block v-else>
                                <view @click.stop="goto_shop(item.shop_supplier_id)" style="border: 1px #F63E36 solid;border-radius: 50rpx;">
                                    <view style="color: #F63E36;height: 60rpx;line-height: 60rpx;font-size: 24rpx;margin: 0 17rpx">
                                        进店逛逛<u-icon name="arrow-right" size="26" color="#F63E36" style="margin-left: 5rpx"></u-icon>
                                    </view>
                                </view>
                            </block>
                    ...
                    <view style="display: flex;margin-top: 20rpx">
                        <block v-if="item.productList.length>0" v-for="(goods,gi) in item.productList">

                            <view v-if="gi<3" style="width: 33%;padding: 10rpx" @click="goto_product(goods.product_id)">
                                <image :src="goods.image[0].file_path" style="width: 100%;height: 200rpx;border-radius: 20rpx" mode="aspectFill"></image>
                                <view style="font-weight: bold" class="linedot">
                                    {{goods.product_name}}
                                </view>
                                <view style="display: flex;align-items: center">
                                    <view style="color: #F63E36;font-size: 28rpx">
                                        ¥{{goods.product_price}}
                                    </view>
                                    <view class='huaxianjia' style="font-size: 24rpx;color: #999999;margin-left: 15rpx">
                                        ¥{{goods.line_price}}
                                    </view>
                                </view>
                            </view>
		...
		methods: {
            ...
            guanzhu(item,index) {
                ...
                self._post('user.Favorite/add', {
                    pid: item.shop_supplier_id,
                    type: 10
                }, (res) => {
                    self.shopData[index].isfollow = 1;
			...
			getData() {
				...
				self.loading = true;
				self._post('purveyor.index/list', {
					page: page || 1,
					list_rows: list_rows,
			...
			goto_shop(shop_supplier_id) {
				this.gotoPage('/pagesShop/shop/shop?shop_supplier_id=' + shop_supplier_id);
			},
			//跳转商品页面
