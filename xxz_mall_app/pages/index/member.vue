...
<!--		<header-bar></header-bar>-->
        ...
        <block v-if="detail.user_id>0" >
            <u-navbar :background="background"  title="我的"  :isBack="false">
                <view slot="left">
                    <image @click="scanQrcode" src="/static/images/member/scan.png" style="width: 40rpx;margin-left: 30rpx" mode="widthFix"></image>
<!--                    <view class="top_search_right" @click="scanQrcode"><text class="icon iconfont icon-saoyisao1"></text></view>-->
		...
		<view v-if="!loadding">
            <view :class="member.is_agent==false&&detail.user_id>0?'bandline':'disheight'" style="height: 360rpx;position: relative;overflow: hidden;">
<!--                <view >
                    <image  src="http://img.pighack.com/202203182253329cf795124.png" style="width: 750rpx;" mode="widthFix"></image>
                </view>-->
                <view class="user-header"  style="position: absolute;top: 0;right: 0;width: 100%;">    <!-- background-color: #F63E36;-->
                    <view v-if="member.is_agent==true" @click="actionshare=true" style="position: absolute;top: 50rpx;right: 0;width: 145rpx;display: flex;background-color: #F63E36;padding: 5rpx 20rpx;border-bottom-left-radius: 30rpx;border-top-left-radius: 30rpx">
                        <image src="/static/images/member/share.png" mode="widthFix" style="width: 40rpx;height: 40rpx"></image>
                        ...
                        <view>
                            ...
                            <view style="display: flex;align-items: center">
                                ...
                                <view>
                                    ...
                                    <u-tag v-if="member.user_id>0 && member.is_agent==true" :text="'推广码  '+detail.user_id" style="margin:0 10rpx" color="#333333" type="success" mode="dark" shape="circle" bg-color="#FFFFFF" />
                                </view>
                            </view>
                        </view>
                    </view>
                    ...
                    <view style="display: flex;justify-content: space-around;align-items: center;width: 90%;margin: auto">
						...
						<view v-if="setting.is_grow === '1'" @click="jumpPage('/pages/user/points/grow')" style="width: 25%;text-align: center;position: relative">
						    <view style="font-size: 35rpx;font-weight: bold">
						        {{ detail.growth_value }}
						    </view>
						    <view>
						        {{ setting.growth_name }} <image src="/static/images/order/arrow.png" style="width: 25rpx;margin-left: 5rpx;margin-bottom: 6rpx;display: inline-block" mode="widthFix"></image>
						    </view>
						</view>
                        <view style="width: 25%;text-align: center;position: relative">
                            <view  style="font-size: 35rpx;font-weight: bold">
                                {{ cardvoucherNum }}
                            </view>
                            <view>
                                卡券 <image src="/static/images/order/arrow.png" style="width: 25rpx;margin-left: 5rpx;margin-bottom: 6rpx" mode="widthFix"></image>
                            </view>
                        </view>
                    ...
                    <view style=" width: 92%;margin: 30rpx auto;position: relative" v-if="member.is_agent==false" @click="jumpPage('/pages/login/comsuccess?self=1')">
                        <!-- <image src="https://img.pighack.com/2022050310120005eed8013.png" style="width: 100%;" mode="widthFix"></image> -->
                        <view style="position: absolute;top: 45rpx;right: 40rpx;color: white;font-size: 22rpx" v-if="member.is_applying==1">
           ...
           <u-popup  v-model="actionshare" mode="bottom" height="" border-radius="50">
            ...
            <u-mask :show="lineshow" @click="lineshow = false">
                <view class="" style="text-align: center">
                    <image :src="img" style="width: 65%;margin:300rpx auto 30rpx;border-radius:20rpx " mode="widthFix"></image>
            ...
            <view class="hlbblock30" style="">
                ...
                <view v-if="detail.user_id>0" style="display: flex;justify-content: space-around;align-items: center;padding-top: 20rpx;border-top: 1px solid #f3f3f3;margin-top: 20rpx">
                    ...
                    <view style="text-align: center;position: relative" @click="jumpPage('/pages/index/order/myorder?dataType=refund')">
      ...
      //#endif
		...
		methods: {
			...
			getData() {
				...
				self._get('user.index/detail', {
                    ...
                    if (Object.keys(res.data.userInfo).length>0){
						...
						// 当会员成为分销商时，显示分销商等级
						if (res.data.is_agent && res.data.agentInfo && res.data.agentInfo.grade) {
			...
			goToLogin() {
				//#ifdef H5
				 if (this.isWeixin()) {
				 	window.location.href = this.websiteUrl + '/index.php/api/user.usermp/login?app_id=' + this.app_id +
				 		'&referee_id=';
				 } else {
            ...
            onShareAppMessage() {
                ...
                return wxShare({id:self.member.user_id});
            },
			getPhoneNumber(e) {
				var self = this;
                ...
                uni.scanCode({
                    ...
                    success: function(res) {
                        ...
                        if (res.errMsg == 'scanCode:ok') {
                          ...
                          if (self.$u.test.jsonString(res.result)){
                            let ts =JSON.parse(res.result);
                            self._post(
                                'store.Order/setOrderVerify', {
                                  verify_code: ts.verify_code,
                                  type: ts.type,
                                },
                                function(res) {
                                  console.log(res)
...
<style lang="scss">
	...
	.asset .time {
		...
		box-shadow: 0px 12rpx 24rpx rgba(0,0,0,0.04);
		border-radius: 20rpx;
	...
	.asset .cfp .item .icon,
	.asset .time .item .icon {
		width: 16rpx;
		height: 16rpx;
		border: 4rpx solid #F63E36;
		border-radius: 50%;
		margin-right: 10rpx;
	}
	.asset .cfp .item .title,
	.asset .time .item .title {
		font-size: 22rpx;
		color: #999999;
	}
	.asset .cfp .value,
