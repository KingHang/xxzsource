		...
		<scroll-view scroll-y="true" class="scroll-Y scroll-box" :style="'height:' + scrollviewHigh + 'px;'" v-if="!loadding">
			...
			<view class="product-pic">
				...
				>
					...
					<swiper-item v-for="(item, index) in detail.image" :key="index"><image @click="yulan(detail.image,index)" :src="item.file_path" mode="aspectFit"></image></swiper-item>
				</swiper>
			</view>
	...
	methods: {
		...
		openService() {
			...
			if (this.service_type == 20) {
				this.gotoPage(
					'/pages/plugin/chat/chat?user_id=' +
