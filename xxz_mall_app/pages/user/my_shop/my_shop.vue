	...
	<view class="weidian">
		...
		<view class="weidian_body">
			...
			<view class="weidian_body_menu_list">
				...
				<view class="weidian_body_menu_list_item" @click="gotoPage('/pagesChat/chat/supplier_chat_list')">
					<view class="weidian_body_menu_list_left_icon">
		...
		methods: {
			...
			get_user_Data() {
				...
				self._get('user.index/detail', {
					...
					self.menu_list[0].url = '/pagesShop/shop/shop?shop_supplier_id=' + self.shop_supplier_id;
					uni.hideLoading()
				})
			},
			// 获取店铺成交数据
			get_shop_Data() {
				let self = this;
				self._get('purveyor.index/tradeData', {}, function(res) {
