			...
			<view :class="{ pb100: tableData.length > 0 }" style="position: relative;">
							...
							<view class="supplier_list">
								...
								<view class="item" v-for="(item, index) in supplier_item.productList" :key="index">
									<label class="d-c-c" @tap.stop="checkItem(item, supplier_index, index)">
										<checkbox color="red" value="cb" class="checkbox" :checked="item.checked" /></label>
				...
				<template v-else>
					<view class="none-data-box cart_none">
						...
						<button type="default" class="btn-gcred mt30 ww100 none_btn" @tap="gotoShop">去购物</button>
					</view>
				</template>
				...
				<view class="bottom-btns f28" v-if="tableData.length > 0">
					...
					<view class="d-e-c pr20" v-if="!isEdit">
						...
						<button type="primary" class="buy-btn" @tap="Submit()">去结算</button>
					</view>
					<view class="pr20" v-else><button type="primary" @tap="onDelete()" class="delete-btn mr20">删除</button></view>
				</view>
