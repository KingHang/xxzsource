			...
			<view class="list">
				<view class="item bg-white p30 mb20" v-for="(item,index) in tableData" :key="index">
					...
					<view class="d-e-c mt20 pt20 border-t">
						<button v-if="item.orderproduct.is_agent==0 && item.plate_status.value==0 && (item.status.value==0 || item.status.value==10) " style="margin-right: 15rpx;" type="default" class="btn-gray-border" @click="intervention(item.order_refund_id)">申请平台介入</button>
						<text v-if="item.orderproduct.is_agent==0 && item.plate_status.value==10 && (item.status.value==0 || item.status.value==10) " style="margin-right: 15rpx;" type="default" class="text_red">平台介入处理中</text>
