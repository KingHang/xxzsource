...
use app\common\model\mall\Access;
use think\facade\Cache;
use think\facade\Session;
use app\mall\model\auth\User;
...
class AuthService
{
    ...
    {
        // 商家登录信息
        $this->store = Session::get('xxzmall_store');
        // 当前用户信息
        $this->user = User::detail($this->store['user']['mall_user_id']);
    }
    ...
    private function getAccessUrls()
    {
        if (empty($this->accessUrls)) {
            // 获取当前用户的角色集
            $roleIds = UserRole::getRoleIds($this->user['mall_user_id']);
            // 根据已分配的权限
