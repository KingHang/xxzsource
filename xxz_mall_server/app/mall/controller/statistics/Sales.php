...
namespace app\mall\controller\statistics;
...
class Sales extends Controller
{
    ...
    public function index()
    {
        return $this->renderSuccess('', [
            ...
            // 成交商品统计
            'goods' => (new OrderService(0))->getProductData(),
            // 销量top10
     ...
     */
    public function SaleRanking($type = 'sale')
    {
        if ($type == 'sale') {
            $search_time = $this->postData('search_time') ? $this->postData('search_time') : '';
            $data = (new ProductRankingService())->getSaleRanking(0,$search_time);
        } elseif ($type == 'view') {
            $data = (new ProductRankingService())->getViewRanking();
        } else {
            $data = (new ProductRankingService())->getRefundRanking();
        }
