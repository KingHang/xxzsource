...
use app\mall\model\auth\User as UserModel;
use app\mall\model\auth\Role;
use app\mall\model\auth\User as AuthUserModel;
use Exception;
...
class User extends Controller
{
     ...
     * @return \think\response\Json
     */
    public function editInfo($mall_user_id)
    {
        $info = UserModel::detail(['mall_user_id' => $mall_user_id], ['UserRole']);
     ...
     * @return \think\response\Json
     */
    public function edit($mall_user_id)
    {
        $data = $this->postData();
        if($this->request->isGet()){
            return $this->editInfo($mall_user_id);
        }

        $model = new UserModel();
        $num = $model->getUserName(['user_name' => $data['user_name']], $data['mall_user_id']);
        if ($num > 0) {
            return $this->renderError('用户名已存在');
        }
        ...
        // 更新记录
        if ($model->edit($data, ['mall_user_id' => $data['mall_user_id']])) {
     ...
     * 删除
     */
    public function delete($mall_user_id)
    {
        $model = new UserModel();
        if ($model->del(['mall_user_id' => $mall_user_id])) {
    ...
    public function getRoleList()
    {
        ...
        $user_info = AuthUserModel::find($user['mall_user_id']);
        $is_need = 0;
        if ($user_info['is_super'] == 1) {
            $roleIds = App::detail($user_info['app_id']);
            $model = new AccessModel();
            if ($roleIds && $roleIds->trade_id) {
                $menus = $model->getListByUser($user['mall_user_id'], '0', $roleIds->trade_id);
                $is_need = 1;
            } else {
                $menus = $model->getList();
            }
            ...
            $model = new AccessModel();
            $menus = $model->getListByUser($user['mall_user_id']);
            $is_need = 1;
        }
