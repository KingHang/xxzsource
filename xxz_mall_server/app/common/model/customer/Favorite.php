...
class Favorite extends BaseModel
{
    ...
    public function getMySupplier($where,$param){
        ...
        $list = $this->alias('s')->where($where)
                        ...
                        ->field("*,(select coalesce(sum(sales_initial+sales_actual),0) from xxzmall_goods p where p.purveyor_id=s.pid) as product_sales")
                        ->order('create_time desc')
                        ->paginate($param, false, ['query' => \request()->request()]);
        foreach ($list as $key => &$value) {
           ...
           //获取最新上架商品
            $productList = $product->with(['image.file'])->where(['product_status'=>10,'shop_supplier_id'=>$value['shop_supplier_id'],'audit_status'=>10,'is_delete'=>0])->field("product_name,product_id,sales_initial,sales_actual,product_price,line_price")->order('product_id desc')->limit(3)->select();
    ...
    {
        return $this->hasOne('app\\common\\model\\purveyor\\Purveyor','shop_supplier_id','pid')->hidden(['user_id','password','total_money','money','freeze_money','cash_money','is_delete','app_id','create_time','update_time']);
    }
