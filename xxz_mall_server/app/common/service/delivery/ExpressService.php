...
class ExpressService
{
    ...
    /**
     * 根据用户收货城市id 验证是否在商品配送规则中 返回商品列表
     */
    public function getNotInRuleProductList()
    {
        $productList = array();
        if ($this->cityId) {
            foreach ($this->productList as &$product) {
                ...
                $cityIds = [];
                $delivery = Delivery::detail($product['delivery_id'],'rule');

                // 判断区域是否支持配送  如未设置区域则全区域配送
                if ($delivery['rule']) {
                    foreach ($delivery['rule'] as $item) {
                        $cityIds = array_merge($cityIds, $item['region_data']);
                        if (!in_array($this->cityId, $cityIds)) {
                            $backProduct['is_support'] = 0;
                        }
                    }
                }
                $productList[] = $backProduct;
            }
        }
        return $productList;
    }
