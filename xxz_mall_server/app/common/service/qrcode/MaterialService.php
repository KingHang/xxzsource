...
//        20 => 'pages/goods/detail/detail',
    ...
    public function getImage()
    {
        ...
        if($this->source == 'wx'){
            // 小程序码参数
            $scene = "gid:{$this->product['material_id']},uid:" . ($this->user_id ?: '');
            // 下载小程序码
    ...
    private function savePoster($backdrop, $productUrl, $qrcode)
    {
        ...
        if ($this->product['branddaysign']['signLog']) {
            $top = 820;
            foreach ($this->product['branddaysign']['signLog'] as $key => $item) {
        ...
        $fontSize = 30;
        $priceType = [10 => 'product_price',20 => 'product_price',30 => 'product_price',40 => 'product_price',50 => 'product_price',60=>'product_price'];
        $priceGood = $this->wrapText($fontSize, 0, $fontPath, $this->product['product']['sku'][0][$priceType[$this->productType]], 850, 80);
        ...
        $fontSize = 30;
        $numGood = $this->wrapText($fontSize, 0, $fontPath, $this->product['product_num']."件单品", 850, 80);
    ...
    {
        return 'product_' . md5("{$this->user_id}_{$this->productType}_{$this->product['material_id']}") . '.png';
    }
