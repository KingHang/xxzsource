...
class MessageService
{
     ...
     * 分销商提现审核通知
     */
    public function activityRemind($activity)
    {
        $message = MessageModel::detailByEname('activity_remind_user');
        $settings = MessageSettingsModel::detailByMessageId($message['message_id']);
        if (!$settings) {
            return;
        }
        ...
        $data = [
            ...
            //报名时间
            'signup_time_start' => $activity['activity']['signup_time_start'],
            // 地址
        ...
        //发送小程序订阅消息
        if ($settings['wx_status'] == 1 && $user['open_id'] != '') {
            $page = 'pages/activity/activity/detail?id=' . $activity['activity_id'];
