...
class WxPay
{
     ...
     * 统一下单API
     */
    public function unifiedorder($order_no, $openid, $totalFee, $orderType = OrderTypeEnum::MASTER, $pay_source, $multiple, $method = 1)
    {
        $data = [
            'attach' => json_encode(['order_type' => $orderType, 'pay_source' => $pay_source, 'multiple' => $multiple, 'method' => $method]),
            'body' => $order_no,
    ...
    public function notify()
    {
        ...
        } else if($attach['pay_source'] == 'wx' || $attach['pay_source'] == 'qr'){
            $this->app = AppWx::getWxPayApp($app_id);
            if($attach['method']==2 && $attach['pay_source'] == 'wx'){
                $this->app = AppWx::getFwxPayApp($app_id);
            }
    ...
    public function refund($transaction_id, $total_fee, $refund_fee)
    {
        // 当前时间
        $time = time();
        $result = $this->app->refund->byTransactionId($transaction_id, $time, intval(strval($total_fee * 100)), intval(strval($refund_fee * 100)), [
