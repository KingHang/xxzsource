...
class Merch_EweiShopV2Page extends AppMobilePage
{
	...
	public function goods_list()
	{
		global $_GPC;
		global $_W;
		$args = array('pagesize' => intval($_GPC['pagesize']), 'home' => intval($_GPC['home']), 'isnew' => trim($_GPC['isnew']), 'ishot' => trim($_GPC['ishot']), 'isrecommand' => trim($_GPC['isrecommand']), 'isdiscount' => trim($_GPC['isdiscount']), 'istime' => trim($_GPC['istime']), 'keywords' => trim($_GPC['keywords']), 'cate' => intval($_GPC['cate']), 'order' => trim($_GPC['order']), 'by' => trim($_GPC['by']));
		//$merch_plugin = p('merch');
		//$merch_data = m('common')->getPluginset('merch');
		//if ($merch_plugin && $merch_data['is_openmerch']) {
