...
class Pay_EweiShopV2Page extends AppMobilePage
{
    ...
    public function main()
    {
        ...
        if (!empty($set["pay"]["wxapp"]) && 0 < $order["price"] ) {
            $tid = $order["ordersn"];
...
//                $wechat = array("success" => false);
        ...
        return app_json(array('jssdkconfig'=>$jssdkconfig,'wechath5' => $wechath5,"order" => array("id" => $order["id"], "ordersn" => $order["ordersn"], "price" => $order["price"], "title" => $set["shop"]["name"] . "订单"), "credit" => $credit, "wechat" => $wechat, "alipay" => $alipay, "cash" => $cash));
    }
    public function complete()
    {
        ...
        $order_goods = pdo_fetchall("select og.id,g.title, og.goodsid,og.optionid,g.stock ,og.total as buycount,g.status from  " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_groupearn_goods") . " g on og.goodsid = g.id " . " where og.orderid=:orderid and og.uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"], ":orderid" => $orderid));
        ...
        if ($type == "credit") {
            ...
            $ban = $ps["fee"] - $credits;
            if ($merge_money < $ps["fee"] || $credits < 0) {
            ...
            if($credits>=$ps["fee"]){
                ...
                $result = m("member")->setCredit($openid, "credit2", 0 - $credits, array($_W["member"]["uid"], $shopset["name"] . "APP 消费" . $credits));
                m("member")->setBalance($openid, 0 - $ban, array($_W["member"]["uid"], $shopset["name"] . "APP 消费" . $ban));
            }
            ...
            $pay_result = m("order")->payResult($ret);
            pdo_update("ewei_shop_order", array('balance'=>$ban), array("id" => $order["id"]));
            return $this->success($orderid);
        }
        if ($type == "wechat") {
    ...
    protected function success($orderid)
    {
        ...
        if (0 < $order["seckilldiscountprice"]) {
            ...
            if (!empty($page)) {
                ...
                $seckill_color = $diydata["home"]["seckill"]["color"];
            }
        }
