...
class extend_EweiShopV2Page extends AppMobilePage {
    ...
    public function __construct() {
        ...
        if(!empty($member['status']) && !empty($member['isagent'])){
            $agentLevel = p('extend')->getLevel($member['openid']);
            $set = p('extend')->getSet();
    ...
    public function main() {
      ...
      $monthorders = pdo_fetchall('select * from ' . tablename('ewei_shop_order') . ' where agentarea=:agentarea and uniacid=:uniacid and createtime>= '.$starttime, array(':agentarea' => $member['agentarea'], ':uniacid' => $_W[uniacid]));
        $orders = pdo_fetchall('select * from ' . tablename('ewei_shop_order') . ' where agentarea=:agentarea and uniacid=:uniacid  ', array(':agentarea' => $member['agentarea'], ':uniacid' => $_W[uniacid]));
         ...
         $monthmoney = 0;
          ...
          $year = date('Y');
          $month = intval(date('m'));
          $bonusall = p('extend')->getBonusData($year, $month, 0, $member['openid']);
          $bonudata = p('extend')->getBonus( $member['openid'],array('ok', 'lock', 'total'));
    ...
    public function down(){
      ...
      public function log(){
            ...
            $bonus = pdo_fetchall('select *  from ' . tablename('ewei_shop_extend_billp') . ' where openid=:openid  and uniacid=:uniacid and createtime >= ' . $starttime . ' and  createtime <=' . $endtime, array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));


            foreach ($bonus as &$b) {

                  $bill = pdo_fetch('select * from ' . tablename('ewei_shop_extend_bill') . ' where id=:id and uniacid=:uniacid   limit 1', array(':id' => $b['billid'], ':uniacid' => $_W[uniacid]));
