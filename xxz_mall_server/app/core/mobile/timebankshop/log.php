...
class Log_EweiShopV2Page extends AppMobilePage
{
    public function getlist()
    {
        ...
        $sql = "SELECT COUNT(*) FROM " . tablename("ewei_shop_timebankshop_log") . " log\r\n                left join " . tablename("ewei_shop_timebankshop_goods") . " g on log.goodsid = g.id\r\n                where 1 " . $condition;
        $total = pdo_fetchcolumn($sql, $params);
        $list = array();
        if (!empty($total)) {
            $sql = "SELECT log.id,log.logno,log.goodsid,log.goods_num,log.status,log.eno,log.paystatus,g.title,g.type,g.thumb,log.credit,log.money,log.dispatch,g.isverify,g.goodstype,log.addressid,log.storeid," . "g.goodstype,log.time_send,log.time_finish,log.iscomment,op.title as optiontitleg,g.merchid " . " FROM " . tablename("ewei_shop_timebankshop_log") . " log " . " left join " . tablename("ewei_shop_timebankshop_goods") . " g on log.goodsid = g.id " . " left join " . tablename("ewei_shop_timebankshop_option") . " op on op.id = log.optionid " . " where 1 " . $condition . " ORDER BY log.createtime DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize;
    ...
    public function detail()
    {
        ...
        $id = intval($_GPC["id"]);
        $log = pdo_fetch("select * from " . tablename("ewei_shop_timebankshop_log") . " where id=:id and openid=:openid " . $condition . " limit 1", array(":id" => $id, ":openid" => $openid, ":uniacid" => $uniacid));
        ...
        $goods = p("timebankshop")->getGoods($log["goodsid"], $member, $log["optionid"]);
        ...
        if (!empty($log["addressid"])) {
            ...
            $goods["dispatch"] = p("timebankshop")->dispatchPrice($log["goodsid"], $log["addressid"], $log["optionid"], $log["goods_num"]);
        }
        ...
        if (!empty($goods["isverify"])) {
            $verifytotal = pdo_fetchcolumn("select count(1) from " . tablename("ewei_shop_timebankshop_verify") . " where logid = :id and openid=:openid " . $condition . " and verifycode = :verifycode ", array(":id" => $id, ":openid" => $log["openid"], ":uniacid" => $log["uniacid"], ":verifycode" => $log["eno"]));
            if ($goods["verifytype"] == 0) {
                $verify = pdo_fetch("select isverify from " . tablename("ewei_shop_timebankshop_verify") . " where logid = :id and openid=:openid " . $condition . " and verifycode = :verifycode ", array(":id" => $log["id"], ":openid" => $log["openid"], ":uniacid" => $log["uniacid"], ":verifycode" => $log["eno"]));
            }
    ...
    public function Receivepacket()
    {
        ...
        $log = pdo_fetch("select * from " . tablename("ewei_shop_timebankshop_log") . " where id = " . $logid . " " . $condition . " ");
        if (!$log) {
        ...
        $packet = p("timebankshop")->packetmoney($log["goodsid"]);
        if (!$packet["status"]) {
        ...
        $goods = pdo_fetch("select surplusmoney from " . tablename("ewei_shop_timebankshop_goods") . " where id = " . $log["goodsid"] . " " . $condition . " ");
        ...
        pdo_update("ewei_shop_timebankshop_goods", $updategoods, array("id" => $log["goodsid"]));
        return app_json();
    }
    public function finish()
    {
        ...
        $log = pdo_fetch("select * from " . tablename("ewei_shop_timebankshop_log") . " where id=:id " . $condition . " and openid=:openid limit 1", array(":id" => $logid, ":uniacid" => $_W["uniacid"], ":openid" => $openidid["openid"]));
        if (empty($log)) {
        ...
        pdo_update("ewei_shop_timebankshop_log", array("time_finish" => time()), array("id" => $logid, "uniacid" => $_W["uniacid"]));
		return app_json(array('id' => $_GPC["id"]));
       // return app_json();
    }
    public function paydispatch()
    {
        ...
        $goods = p("timebankshop")->getGoods($log["goodsid"], $member);
        if (empty($goods["id"])) {
        ...
        if ($goods["dispatch"] <= 0) {
            pdo_update("ewei_shop_timebankshop_log", array("dispatchstatus" => 1, "addressid" => $addressid), array("id" => $log["id"]));
        ...
        if ($paytype == "wechat") {
            ...
            if (empty($dispatchno)) {
                ...
                pdo_update("ewei_shop_timebankshop_log", array("dispatchno" => $dispatchno, "addressid" => $addressid, "dispatch" => $dispatchprice), array("id" => $log["id"]));
            }
            $tid = $dispatchno;
    ...
    public function paydispatchresult()
    {
        ...
        pdo_update("ewei_shop_timebankshop_log", array("dispatchstatus" => 1), array("id" => $log["id"]));
        return app_json();
    }
    public function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename("ewei_shop_timebankshop_log") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
        if (empty($order)) {
            return app_error(AppError::$OrderNotFound);
        }
        ...
        $goods = pdo_fetchall("select og.goodsid,g.thumb  from " . tablename("ewei_shop_timebankshop_log") . " og " . " left join " . tablename("ewei_shop_timebankshop_goods") . " g on g.id=og.goodsid " . " where og.id=:orderid and og.uniacid=:uniacid ", array(":uniacid" => $uniacid, ":orderid" => $orderid));
