...
class Activity_EweiShopV2Page extends AppMobilePage {
    ...
    public function get_category() {
        ...
        $sql = 'select  *  from ' . tablename('ewei_shop_activity_category') . '   where  uniacid=:uniacid order by displayorder desc';
        ...
        return app_json(array('list' => $listss));
    }
    ...
    public function get_banner() {
		...
		$advs = pdo_fetchall('select id,advname,link,thumb from ' . tablename('ewei_shop_activity_adv') . ' where ' . $contation . ' and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	...
	public function get_list()
      {
		global $_W;
		global $_GPC;
		$page = intval($_GPC['home']);
		$pindex = max(1, $page);
		$psize = 20;
		
		if ($_GPC['id']>0){
			  $condition = ' and `uniacid` = :uniacid and category=:category';
			  $params = array(':uniacid' => $_W['uniacid'],'category'=>$_GPC['id']);
		}else{
		...
		$sql = 'select * from ' . tablename('ewei_shop_activity') . ('  where 1 and isshow=1 ' . $condition . ' ORDER BY id DESC  LIMIT ' . ($pindex - 1) * $psize . ',' . $psize) ;
		$list = pdo_fetchall($sql, $params);
		$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_activity') . (' where 1 and isshow=1 ' . $condition), $params);
		foreach ($list as &$row){
			  $row['logo']=tomedia($row['logo']);
			  $row['charge']=unserialize($row['charge']);
			  $row['price']=$row['charge'][0]['pprice'];
			  if ($row['chargetype']==0){
					$row['price']='免费';
			  }
			  if ($row['hostid'] > 0) {
					$host = pdo_fetch('select * from ' . tablename('ewei_shop_activity_host') . ' where id=:id ', array(':id' => $row['hostid']));
					$host = set_medias($host, 'logo');
					$row['host']['name'] = $host['name'];
					$row['host']['logo'] = $host['logo'];
				} else {
					$row['host']['name'] = '总部';
					$row['host']['logo'] = 'http://img.dfhlyl.com/375112a2371f03790a065315aa3876bc.png';
				}
		}
		unset($row);
	...
	public function get_details()
      {
        ...
        $activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        $activity['charge'] = unserialize($activity['charge']);
        $activity['price'] = number_format($activity['charge'][0]['pprice'], 2);
        if ($activity['chargetype'] == 0) {
            $activity['price'] = '免费';
            $activity['charge'] = array();
            $activity['charge'][0] = array('pname' => '免费', 'pprice' => 0);
        }
        $count = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_activity_favorite") . " where aid=:aid and deleted=0 and openid=:openid and uniacid=:uniacid limit 1", array(":aid" => $id, ":openid" => $openid, ":uniacid" => $_W["uniacid"]));
        if ($count > 0) {
            $isFavorite = 1;
			$activity['isfavorite'] = 1;
        }else{
			$activity['isfavorite'] = 0;
		}
		//print_r($count);exit;
        if (empty($activity)) {
            $this->message('活动不存在!', mobileUrl('activity/main'), 'error');
        }
        $goodsurl = mobileUrl('order/create', array('id' => $activity['goodsid']));

        $member = m('member')->getMember($openid);
        $_W['shopshare'] = array('title' => $activity['name'], 'imgUrl' => tomedia($activity['logo']), 'desc' => $activity['name'], 'link' => mobileUrl('activity', array('aid' => $activity['id'], 'lockid' => $member['id']), true));


        //报名人数；
        $signnum = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_activity_log") . " where activityid=:activityid  ", array(":activityid" => $activity['id']));
        $signnum += $activity['signnum_v'];
		
        if ($activity['hostid'] > 0) {
            $host = pdo_fetch('select * from ' . tablename('ewei_shop_activity_host') . ' where id=:id ', array(':id' => $activity['hostid']));
			$activity['hostname'] = $host['name'];
        } else {
			$activity['hostname'] = '总部';
            $host['name'] = '总部';
        }
        $endtime = strtotime($activity['activity_time_end']);
        $starttime = strtotime($activity['activity_time_start']);
        if ($endtime > time()) {
            $endday = floor(($endtime - time()) / 86400);
            $hour = floor(($endtime - time()) % 86400 / 3600);
            $endmsg = '剩余:' . $endday . '天' . $hour . '小时';
			$activity['acstop'] = 0;
        } else {
            $endmsg = '活动已结束';
            $acstop = 1;
        ...
        $alllog = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_activity_log') . ' WHERE activityid=:activityid  ', array(':activityid' => $activity['id']));
        if ((count($alllog)) > $activity['total']) {
		...
		if ($activity['total']==0) {
            $activity['shengyu'] = '无限制';
			
 
        }else{
			$activity['shengyu'] = $activity['total']-$signnum;
		}

        $showlogs = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_activity_log') . ' WHERE activityid=:activityid order by id desc limit 45', array(':activityid' => $activity['id']));
        foreach ($showlogs as &$l) {
            $l['avatar'] = pdo_fetchcolumn('select avatar from ' . tablename('ewei_shop_member') . ' where openid=:openid and uniacid=:uniacid   limit 1', array(':openid' => $l['openid'], ':uniacid' => $_W[uniacid]));
        }
        unset($l);
		$activity['signnum'] =$signnum;
		$activity['baoming']['signnum'] =$signnum;
		$activity['baoming']['showlogs'] =$showlogs;
        //会员卡链接
        $group = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity_group') . ' WHERE id=:id   limit 1 ', array(':id' => $activity['groupid']));
        $membercard = iunserializer($group['membercard']);
        if (!empty($membercard)) {
            $membercardurl = mobileUrl('membercard/detail', array('cid' => $membercard[0], 'type' => all));
        }
        //写入非固定分销人员
        $lockid = intval($_GPC['lockid']);
        $thismember = m('member')->getMember($openid);
        if ($lockid != $thismember['id']) {
            pdo_update('ewei_shop_member', array('unlockid' => $lockid), array('id' => $thismember['id'], 'uniacid' => $_W['uniacid']));
        }
        //手机绑定
        $set = m('common')->getPluginset('activity');
        $actbind = $set['tm']['actbind'];

        //协议
        $agrdata = m("common")->getSysset("agreement");
        $agractivitytitle = pdo_fetchcolumn('select title from ' . tablename('ewei_shop_agreement') . ' where id=:id and uniacid=:uniacid   limit 1', array(':id' => $agrdata['activityid'], ':uniacid' => $_W[uniacid]));

        if (empty($activity['lat'])) {
            $activity['lat'] = 0;
        }
        if (empty($activity['lng'])) {
            $activity['lng'] = 0;
        }
		$activity = set_medias($activity, 'logo');
        return app_json(array('list' => $activity));
	  }
	  
	  /**
     * 活动详情中已报名人数
     */
	  public function get_signlog()
      {
		global $_W;
        global $_GPC;
		$id = intval($_GPC['id']);
        $list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_activity_log') . ' WHERE activityid=:activityid  ORDER BY id DESC  ', array(':activityid' => $_GPC['id']));
        foreach ($list as $key => &$item) {
            $item['member'] = m('member')->getMember($item['openid']);
			$lists = m('member')->getMember($item['openid']);
			$listss[$key]['avatar'] = $lists['avatar'];
			$listss[$key]['nickname'] = $lists['nickname'];
        }
		return app_json(array('list' => $listss));
	  }
	 /**
     ...
     */
	  public function isfavorite() {
        ...
        $sql = 'select  *  from ' . tablename('ewei_shop_member') . '   where openid=:openid or openid_wa=:openid_wa' . $condition;
        $openidid = pdo_fetch($sql, $params); 
		//print_r($openidid);exit;
        $activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $id, ':uniacid' => $_W['uniacid']));

        $data = pdo_fetch('select id,deleted from ' . tablename('ewei_shop_activity_favorite') . ' where uniacid=:uniacid and aid=:id and openid=:openid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openidid['openid'], ':id' => $id));

        if (empty($data)) {
            if (!empty($isfavorite)) {
                $data = array('uniacid' => $_W['uniacid'], 'aid' => $id, 'openid' => $openidid['openid'], 'createtime' => time());
                pdo_insert('ewei_shop_activity_favorite', $data);
            }
        } else {
            pdo_update('ewei_shop_activity_favorite', array('deleted' => $isfavorite ? 0 : 1), array('id' => $data['id'], 'uniacid' => $_W['uniacid']));
        }
	...
	public function mymember() {
		...
		$params = array(':openid' => $_GPC['openid'], ':openid_wa' => $_GPC['openid']);
        $condition = '';
        $sql = 'select  id,openid,openid_wa,avatar,credit2,nickname,agentarea,credit1,level  from ' . tablename('ewei_shop_member') . '   where openid=:openid or openid_wa=:openid_wa' . $condition;
        $member = pdo_fetch($sql, $params);
		
	    //$member = m('member')->getMember($_W['openid']);
	    $member["order_0"] = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid  and status=0 and  isactivity=1 and uniacid=:uniacid", array(':openid' => $member['openid'],':uniacid' => $_W[uniacid]));  //待付款
	    $member["order_1"] = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid  and status=1 and  isactivity=1 and uniacid=:uniacid", array(':openid' => $member['openid'],':uniacid' => $_W[uniacid]));   //已支付
	    $member["order_3"] = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid  and status=3 and  isactivity=1 and uniacid=:uniacid", array(':openid' => $member['openid'],':uniacid' => $_W[uniacid]));    //已完成
		if($member['level'] == 0){
			$member['levelname'] ='普通会员';
		}
		return app_json(array('list' => $member));
	}
	/**
     * 报名记录
     */
	 public function log_get_list() {
		...
		if( $show_status != 3 )
		{
			...
			switch( $show_status ) 
			{
				case 0: $condition .= " and status=0";
				break;
				case 1: $condition .= " and status=1 ";
				break;
		...
		$list = pdo_fetchall("select * from " . tablename("ewei_shop_activity_log") . " where 1 " . $condition . " order by createtime desc LIMIT " . ($pindex - 1) * $psize . "," . $psize, $params);
        $i=0;
		$total = pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_activity_log") . " where 1 " . $condition, $params);
         foreach ($list as &$row){
			 $i++;
               $row['order']=pdo_fetch('select id,pname,price from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid   limit 1', array(':id' => $row['orderid'], ':uniacid' => $_W[uniacid]));
               $row['activity']= pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $row['activityid'], ':uniacid' => $_W['uniacid']));
                  $row['start'] =date('Y.m.d H:i',strtotime($row['activity']['activity_time_start']));
                  $row['end'] =date('Y.m.d H:i',strtotime($row['activity']['activity_time_end']));
               $row['address'] =$row['activity']['province'].$row['activity']['city'].$row['activity']['area'];
               $row['activity']['logo']=tomedia($row['activity']['logo']);
               $row['activity']['host']=  pdo_fetch('select * from ' . tablename('ewei_shop_activity_host') . ' where id=:id ', array(':id' => $row['activity']['hostid']));
                  if ($row['activity']['hostid']<1){
                        $row['activity']['host']['name']='总部';
                  }
     ...
     */
	 public function log_detail() {
		...
		$log = pdo_fetch('select * from ' . tablename('ewei_shop_activity_log') . ' where id=:id  limit 1', array(':id' => $logid));

		$activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $log['activityid'], ':uniacid' => $_W['uniacid']));
		$activity['logo']=tomedia($activity['logo']);
		$order = pdo_fetch("select * from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array( ":id" => $log['orderid'], ":uniacid" => $uniacid, ":openid" => $openid ));

		$query = array('id' => $logid, 'verifycode' => $log['verifycode']);

					 $url = mobileUrl('activity/hostcheck', $query, true);

					$verifycode = $log["verifycode"];
					$qrcodeimg = m("qrcode")->createQrcode($url);
		$starttime =$activity['activity_time_start'].' 开始';
		$address =$activity['province'].$activity['city'].$activity['area'].$activity['street'];
		if (empty($log['actsite'])){
			  $site='随机';
		}else{
			  $site=$log['actsite'];
		}
		$list['activity'] = $activity;    //活动详情
		$list['qrcodeimg'] = $qrcodeimg; //二维码
		$list['verifycode'] = $verifycode;  //核验码
		$list['site'] = $site;   //座号
		$list['starttime'] = $starttime;    //活动时间
		$list['address'] = $address;  // 活动地点
		return app_json(array('list' => $list));
	 }
     ...
     */
	 public function log_qrcode() {
		...
		$query = array('id' => $logid, 'verifycode' => $log['verifycode']);
		$verifycode = $log["verifycode"];
		$order = pdo_fetch('select activityid from ' . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['orderid'], ':uniacid' => $_W['uniacid']));
		$activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $order['activityid'], ':uniacid' => $_W['uniacid']));
		$starttime =$activity['activity_time_start'].' 活动开始';
		$address =$activity['province'].$activity['city'].$activity['area'].$activity['street'];
		if (empty($log['actsite'])){
			  $site='坐席：随机';
		}else{
		...
		$list['qrcodeimg'] = m('qrcode')->createQrcode($url); //二维码
		$list['verifycode'] = $verifycode;  //核验码
		$list['site'] = $site;   //座号
		$list['starttime'] = $starttime;    //活动时间
		$list['address'] = $address;  // 活动地点
		
		//show_json(1, array('url' => m('qrcode')->createQrcode($url),'starttime'=>$starttime,'address'=>$address,'site'=>$site));
		return app_json(array('list' => $list));
	 }
	 /**
     * 我的关注
	...
	public function myfavorite() {
		...
		$params = array(':uniacid' => $_W['uniacid'], ':openid' => $openidid['openid']);
		$sql = 'SELECT COUNT(*) FROM ' . tablename('ewei_shop_activity_favorite') . (' f where 1 ' . $condition);
		$total = pdo_fetchcolumn($sql, $params);
		$list = array();
		if (!empty($total)) {
			$sql = 'SELECT a.* FROM ' . tablename('ewei_shop_activity_favorite') . ' f ' . ' left join ' . tablename('ewei_shop_activity') . ' a on f.aid = a.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
			$list = pdo_fetchall($sql, $params);
			$list = set_medias($list, 'logo');
		...
		$i=0;
          foreach ($list as &$li){
				$li['start']=strtotime($li['activity_time_start']);
				$li['end']=strtotime($li['activity_time_end']);
				if ($time>$li['start']&&$time<$li['end']){
					$li['stmsg']='1';
				}
				$i++;
		  }
	...
	public function activity_order() {
		...
		$show_status = $_GPC["status"];
		$r_type = array( "退款", "退货退款", "换货" );
		$condition = " and openid=:openid and ismr=0 and deleted=0 and uniacid=:uniacid and istrade=0 and isactivity=1";
		...
		if( $show_status != 8 ) 
		{
			...
			switch( $show_status ) 
			{
				...
				break;
				case 4: $condition .= " and refundstate=1";
				break;
				case 5: $condition .= " and refundid>0 and refundstate=0 ";
				break;
				default: $condition .= " and status=" . intval($show_status);
			}
			if( $show_status != 5 ) 
	...
	public function activity_submit() 
	{
		...
		$sql = "SELECT diyformtype,diyfields,pricedown,diyformid FROM " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid  limit 1";
		...
		if( p("threen") ) 
		{
			$threenvip = p("threen")->getMember($openidid["openid"]);
			if( !empty($threenvip) ) 
			{
				$threenprice = true;
			}
		...
		foreach( $goods as $g ) 
		{
			...
			$data["seckillinfo"] = plugin_run("seckill::getSeckill", $goodsid, $optionid, true, $openidid["openid"]);
			...
			if( p("task") ) 
			{
				...
				if( !empty($task_id) ) 
				{
					$rewarded = pdo_fetchcolumn("SELECT `rewarded` FROM " . tablename("ewei_shop_task_extension_join") . " WHERE id = :id AND openid = :openid AND uniacid = :uniacid", array( ":id" => $task_id, ":uniacid" => $_W["uniacid"], ":openid" => $openidid["openid"] ));
			...
			else 
			{
				if( $packageid ) 
				{
					if( empty($g["giftStatus"]) ) 
					{
                        ...
                        $data['packagepvprice'] = $pg['pvprice'];
                        if( 0 < $cardid )
						{
		...
		if( !empty($goods[0]["bargain_id"]) && p("pricedown") )
		{
			$bargain_act = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_bargain_actor") . " WHERE id = :id AND openid = :openid ", array( ":id" => $goods[0]["bargain_id"], ":openid" => $openidid["openid"] ));
			...
			if( !pdo_update("ewei_shop_bargain_actor", array( "status" => 1 ), array( "id" => $goods[0]["bargain_id"], "openid" => $openidid["openid"] )) ) 
			{
				exit( "下单失败" );
			}
		...
		if( $taskgoodsprice ) 
		{
			...
			pdo_update("ewei_shop_task_extension_join", array( "rewarded" => serialize($taskGoodsInfo0) ), array( "id" => $task_id, "uniacid" => $_W["uniacid"], "openid" => $openidid["openid"] ));
			unset($_SESSION[$goodsid . "_task_id"]);
		}
		$order = array( );
        $agentarea = pdo_fetch('select agentarea from ' . tablename('ewei_shop_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
        $order['agentarea']=$agentarea['agentarea'];
		...
		else 
		{
			if(  is_array($_GPC["diydata"])  )
			{
				...
				$order["diyformfields"] = iserializer($fields);
				$order["diyformdata"] = $idata;
				$order["diyformdata"] = serialize($_GPC["diydata"]);
				$order["diyformid"] = $order_formInfo["id"];
			}
		}
          $activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $_GPC['activityid'], ':uniacid' => $_W['uniacid']));
           $buylog =pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_activity_log') . ' WHERE activityid=:activityid  and openid=:openid  ', array(':activityid' => $_GPC['activityid'], ':openid' => $openidid['openid']));
           $alllog =pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_activity_log') . ' WHERE activityid=:activityid and status>=0 ', array(':activityid' => $_GPC['activityid'], ':openid' => $openidid['openid']));
            $thisbuy =count($_GPC["diydata"]);
            if (($thisbuy+count($buylog))>$activity['limitbuy']){
                  if ($activity['limitbuy']>0){
                        show_json(0,'门票最多购买'.$activity['limitbuy'].'张，已经购买'.count($buylog).'超出最大购买限制');
                  }
            }
          if (($thisbuy+count($alllog))>$activity['total']){
                  if ($activity['total']>0){
                        $remain = $activity['limitby']-count($alllog);
                        show_json(0,'本次活动名额已满，您可以报名其他活动！');
                  }
          }
          foreach ($_GPC['formmobile'] as $fm){
                  $buylog = pdo_fetch('select * from ' . tablename('ewei_shop_activity_log') . ' where mobile=:mobile and activityid=:activityid   limit 1', array(':mobile' => $fm, ':activityid' => $_GPC['activityid']));
                  if (!empty($buylog)){
                        show_json(0,'号码'.$fm.'已经报名了');
                  }
          }
          $order["diyformfields"] =$activity['diyfields'];
		if( !empty($address) ) 
		{
		...
		if ($_GPC['isknowledge']==1){
		      $order['isknowledge']=1;
        }
          $order['isactivity']=1;
          $order['pname']=$_GPC['pname'];
          $order['activityid']=$_GPC['activityid'];
          $order['formname']=serialize($_GPC['formname']);
          $order['formmobile']=serialize($_GPC['formmobile']);
          $order['fname']=$_GPC['formname'][0];
          $order['fmobile']=$_GPC['formmobile'][0];
          //pand号码唯一

          if (count($_GPC['formmobile']) != count(array_unique($_GPC['formmobile']))) {
                show_json(0,'手机号码有重复');
          }
		...
		{
			p("membercard")->member_card_use_record($orderid, $cardid, $carddeductprice, $openidid["openid"]);
		}
		if( !empty($goods[0]["bargain_id"]) && p("pricedown") )
		{
			pdo_update("ewei_shop_bargain_actor", array( "order" => $orderid ), array( "id" => $goods[0]["bargain_id"], "openid" => $openidid["openid"] ));
		}
		if( $multiple_order == 0 ) 
		{
			...
			{
				$order_goods = array( );
				if( !empty($bargain_act) && p("pricedown") )
				{
					$goods["total"] = 1;
				...
				$order_goods["realprice"] = $goods["marketprice"]*$goods["total"];
                $pvprice= pdo_fetch('select productprice from ' . tablename('ewei_shop_goods') . ' where id=:id    limit 1', array(':id' =>$order_goods['goodsid']));
                $order_goods['pvprice'] =$pvprice['productprice'];
                if (!empty($goods['packagepvprice'])){
                    $order_goods['pvprice']=$goods['packagepvprice'];
                }
				...
				if( $goods["seckillinfo"] && $goods["seckillinfo"]["status"] == 0 ) 
				{
					plugin_run("seckill::setSeckill", $goods["seckillinfo"], $goods, $openidid["openid"], $orderid, 0, $order["createtime"]);
				}
			}
		}
		else 
		...
		if( !is_null($_SESSION["exchangeprice"]) && !empty($_SESSION["exchange"]) && p("exchange") ) 
		{
			$exchangeinfo = m("member")->getInfo($openidid["openid"]);
			if( $groupResult["mode"] != 6 ) 
			{
				$exchangedata = array( "key" => $_SESSION["exchange_key"], "uniacid" => $_W["uniacid"], "goods" => $_SESSION["exchangegoods"], "orderid" => $orderid, "time" => time(), "openid" => $openidid["openid"], "mode" => 1, "nickname" => $exchangeinfo["nickname"], "groupid" => $_SESSION["groupid"], "title" => $_SESSION["exchangetitle"], "serial" => $_SESSION["exchangeserial"], "ordersn" => $order["ordersn"], "goods_title" => $exchangetitle );
				pdo_update("ewei_shop_exchange_record", $exchangedata, array( "key" => $_SESSION["exchange_key"], "uniacid" => $_W["uniacid"] ));
			}
			else 
			{
				$exchangedata = array( "goods" => $_SESSION["exchangegoods"], "orderid" => $orderid, "time" => time(), "openid" => $openidid["openid"], "mode" => 6, "nickname" => $exchangeinfo["nickname"], "ordersn" => $order["ordersn"], "goods_title" => $exchangetitle );
				pdo_update("ewei_shop_exchange_record", $exchangedata, array( "key" => $_SESSION["exchange_key"], "uniacid" => $_W["uniacid"] ));
			}
			pdo_update("ewei_shop_exchange_cart", array( "selected" => 0 ), array( "uniacid" => $_W["uniacid"], "openid" => $openidid["openid"], "selected" => 1 ));
		}
		...
		if( p("quick") && !empty($_GPC["fromquick"]) ) 
		{
			pdo_update("ewei_shop_quick_cart", array( "deleted" => 1 ), array( "quickid" => intval($_GPC["fromquick"]), "uniacid" => $_W["uniacid"], "openid" => $openidid["openid"] ));
		}
		if( 0 < $deductcredit ) 
		{
		...
		$orderss[] = array();
		$orderss = pdo_fetch("select * from " . tablename("ewei_shop_order") . " where id=:id limit 1", array(":id" => $orderid));
        
		return app_json(array('list' => $orderss));
		//show_json(1, array( "list" => $orderss ));
	}
	...
	public function activity_addcreate() {
		global $_W;
        global $_GPC;
		...
		$member = m("member")->getMember($openidid["openid"]);
		$show_card = true;
		if( p("exchange") ) 
		{
		...
		if( !$packageid ) 
		{
			...
			$_SESSION["bargain_id"] = NULL;
			if( p("pricedown") && !empty($bargain_id) )
			{
				
				$show_card = false;
				$_SESSION["bargain_id"] = $bargain_id;
				$bargain_act = pdo_fetch("SELECT *  FROM " . tablename("ewei_shop_bargain_actor") . " WHERE id = :id AND openid = :openid AND status = '0'", array( ":id" => $bargain_id, ":openid" => $openidid["openid"] ));
				if( empty($bargain_act) ) 
				{
					exit( "没有这个商品!" );
				}
				...
				$if_bargain = pdo_fetch("SELECT pricedown FROM " . tablename("ewei_shop_goods") . " WHERE id = :id AND uniacid = :uniacid ", array( ":id" => $bargain_act_id["goods_id"], ":uniacid" => $_W["uniacid"] ));
				if( empty($if_bargain["pricedown"]) )
				{
					exit( "没有这个商品!" );
				}
			...
			if( empty($id) ) 
			{
				...
				foreach( $goods as $k => $v ) 
				{
					...
					$goods[$k]["seckillinfo"] = plugin_run("seckill::getSeckill", $v["goodsid"], $v["optionid"], true, $openidid["openid"]);
			...
			else 
			{
				...
				else 
				{
					...
					$sql = "SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice," . " thumb,marketprice,liveprice,islive,storeids,isverify,isforceverifystore,deduct,hasoption,preselltimeend,presellsendstatrttime,presellsendtime,presellsendtype," . " manydeduct,`virtual`,maxbuy,usermaxbuy,discounts,total as stock,deduct2,showlevels," . " ednum,edmoney,edareas,edareas_code,unite_total,diyfields," . " diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice,cates,minbuy, " . " isdiscount,isdiscount_time,isdiscount_discounts,isfullback, " . " virtualsend,invoice,needfollow,followtip,followurl,merchid,checked,merchsale, " . " buyagain,buyagain_islong,buyagain_condition, buyagain_sale,pricedown" . $threensql . $ishotelsql . " FROM " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid  limit 1";
					$data = pdo_fetch($sql, array( ":uniacid" => $uniacid, ":id" => $id ));
					$threenprice = json_decode($data["threen"], 1);
					if( 0 < $data["pricedown"] )
					{
					...
					$data["seckillinfo"] = plugin_run("seckill::getSeckill", $data["goodsid"], $optionid, true, $openidid["openid"]);
					if( $data["seckillinfo"] ) 
					{
						$show_card = false;
					}
			...
			if( !$isverify && !$isvirtual && !$ismerch ) 
			{
				if( 0 < $merch_id ) 
				{
                    ...
                    $carrier_list = pdo_fetchall("select * from " . tablename("ewei_shop_store") . " where  uniacid=:uniacid and status=1 and type in(1,3) and merchid=:merchid order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merch_id ));
                    $carrier_list= m('order')->checkstockstore($goods, $carrier_list);
                    foreach ($carrier_list as $k=>$ca){
                        if ($ca['inv']<1){
                            unset($carrier_list[$k]);
                        }
                    }
                    unset($k);
                    unset($ca);
			...
			if( $isverify ) 
			{
				...
				if( empty($storeids) ) 
				{
					if( 0 < $merchid ) 
					{
                        ...
                        $stores = pdo_fetchall("select * from " . tablename("ewei_shop_store") . " where merchid=:merchid and uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc", array( ":uniacid" => $_W["uniacid"], ":merchid" => $merchid ));
                        $stores= m('order')->checkstockstore($goods, $stores);
                        foreach ($stores as $k=>$ca){
                            if ($ca['inv']<1){
                                unset($stores[$k]);
                            }
                        }
		...
		else 
		{
			...
			foreach( $g as $key => $value ) 
			{
				...
				{
					$packagegoods = pdo_fetch("select title,packageprice,marketprice,total from " . tablename("ewei_shop_package_goods") . "\r\n                            where goodsid=" . $value["goodsid"] . " and uniacid = " . $uniacid . " and pid = " . $packageid . " ");
					$goods[$key]["packageprice"] = $packagegoods["packageprice"];
                    $goods[$key]['total'] = $packagegoods['total'];
				}
		...
		$goods_list = array( );
		$goods_list[0]["activityid"] = $_GPC['activityid'];
        $goods_list[0]["iprice"] = $_GPC['iprice'];
		$goods_list[0]["total"] = $_GPC['total'];
		$goods_list[0]["pname"] = $_GPC['pname'];
		$goods_list[0]["totalprice"] = $_GPC['iprice']*$_GPC['total'];
		if( $ismerch ) 
		{
			$getListUser = $merch_plugin->getListUser($goods);
			$merch_user = $getListUser["merch_user"];
        ...
        if ($packageid){
            foreach ($goods as &$go){
                $go['type']=pdo_fetchcolumn('select type from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid   limit 1', array(':id' => $go['goodsid'], ':uniacid' => $_W[uniacid]));
            }
        }
        unset($go);
        $type = $goods[0]['type'];
        foreach ($goods as $go){
            if ($type != $g['type']) {
                ...
                $createInfo['isvirtual']=1;
            }
        }
        $allprice=$_GPC['iprice']*$total;
          $createInfo['allprice']=$allprice;
          $activity = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_activity') . ' WHERE id=:id  and uniacid=:uniacid limit 1 ', array(':id' => $_GPC['activityid'], ':uniacid' => $_W['uniacid']));
          $fields=unserialize($activity['diyfields']);

            foreach ($createInfo['goods'] as &$cg){
                  $cg['marketprice']=$_GPC['iprice'];
                  $cg['ggprice']=$_GPC['iprice'];
            }
            unset($cg);
            $mem = m("member")->getMember($openid, true);
			$params2 = array(':openid' => $_GPC['openid'], ':openid_wa' => $_GPC['openid']);
			$condition2 = '';
			$sql2 = 'select  *  from ' . tablename('ewei_shop_member') . '   where openid=:openid or openid_wa=:openid_wa' . $condition2;
			$openidid = pdo_fetch($sql2, $params2);
			$member = m("member")->getMember($openidid["openid"]);
			$member_list['realname'] = $member['realname'];
			$member_list['mobile'] = $member['mobile'];
			return app_json(array('list' => $goods_list,'member' => $member_list));
    ...
    public function type_credit() {
		...
		if($pay_result){
			return app_json(array('list' => $orderid));
			//show_json(1, '支付成功');
		}else{
			show_json(0, '支付失败');
		}
        // return app_json(array('list' => $credits));
	}
