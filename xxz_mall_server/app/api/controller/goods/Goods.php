...
use app\api\model\plugin\voucher\Voucher as CouponModel;
...
//        (new VisitModel())->addVisit($user, $product['supplier'], $params['visitcode'], $product);
        return $this->renderSuccess('', [
            ...
            'specData' => $specData,
            // 微信公众号分享参数
            'share' => $this->getShareParams($url, $product['product_name'], $product['product_name'], '/pages/goods/detail/detail', $product['image'][0]['file_path']),
            ...
            //店铺客服信息
            'mp_service' => ServiceModel::detail($product['purveyor_id']),
        ]);
    }
    ...
    public function poster($product_id, $source,$productType=10)
    {
        ...
        if ($productType==20){
            ...
            $log = \app\common\model\plugin\flashsell\Goods::detail($product_id);//log_id
            $productId = $log->goods_id;
            $detail = ProductModel::detail($productId);
            $detail->product_id = $product_id;
            $Qrcode = new ProductService($detail, $this->getUser(false), $source,$productType);
        }
        ...
        if ($productType==50){
            $log = \app\common\model\plugin\pricedown\Goods::detail($product_id);
            $productId = $log->goods_id;
            $detail = ProductModel::detail($productId);
            $detail->product_id = $product_id;
            $Qrcode = new ProductService($detail, $this->getUser(false), $source,$productType);
