...
use app\common\model\plugin\voucher\Coupon;
...
use app\api\model\purveyor\ServiceApply;
...
class Goods extends ProductModel
{
    ...
    public function getDetails($productId, $userInfo = false)
    {
        ...
        $detail['shop_supplier_id'] = $detail['purveyor_id'];
        $SupplierModel = new SupplierModel;
        if ($detail['purveyor_id']) {
            $supplier = $SupplierModel::detail($detail['purveyor_id'], ['logo', 'category']);
            $supplier['logos'] = $supplier['logo']['file_path'];
            unset($supplier['logo']);
            $supplier['category_name'] = $supplier['category']['name'];
            unset($supplier['category']);
            $supplier['supplier_user_id'] = (new SupplierUserModel())->where('purveyor_id', '=', $detail['purveyor_id'])->value('purveyor_user_id');
            $supplier->visible(['logos', 'category_name', 'name', 'purveyor_id', 'product_sales', 'server_score', 'store_type', 'user_id', 'purveyor_user_id']);
            $server = (new ServiceApply())->getList($detail['purveyor_id']);
        } else {
            $supplier = [];
            $server = [];
        }
        ...
        // 更新访问数据
        $this->where('goods_id', '=', $detail['product_id'])->inc('view_times')->update();
        // 设置商品展示的数据
    ...
    {
        $coupon = Coupon::where('is_delete', '=', 0)->where(['is_receive'=>1])
        ...
        foreach ($coupon as $k => $v)
        {
            ...
            if ($v['apply_range'] == 10)
            {
                $product_coupon_ids.=$v['voucher_id'].',';
            }if ($v['apply_range'] == 20)
            {
    ...
    public static function getUserCouponList($coupon_ids, $orderPayPrice)
    {
        ...
        //     新增筛选条件: 最低消费金额
        // 获取用户可用的优惠券列表
        $list = Coupon::where('voucher_id','in',$coupon_ids)->select();
        if (!$list){
            return 0;
        }
        $data = [];
        foreach ($list as $key =>$coupon) {
            ...
            $data[$key] = [
                ...
                'free_limit' => $coupon['voucher']['free_limit'],
                'apply_range' => $coupon['voucher']['apply_range'],
                'product_ids' => $coupon['voucher']['product_ids'],
            ];
            // 计算打折金额
