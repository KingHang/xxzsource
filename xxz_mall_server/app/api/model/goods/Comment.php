...
use app\common\model\goods\Comment as CommentModel;
...
class Comment extends CommentModel
{
    ...
    private function updateOrderIsComment($order, $isComment, $commentList)
    {
        ...
        foreach ($commentList as $comment) {
            $orderProductData[] = [
                'order_goods_id' => $comment['order_product_id'],
                'is_comment' => 1
            ];
        }
        // 更新订单
        $isComment && $order->save(['is_comment' => 1]);
        return (new OrderGoods)->saveAll($orderProductData);
    }
    ...
    private function createCommentData($user_id, $order_id, $productList, $formData)
    {
        ...
        foreach ($productList as $product) {
            if (!isset($formData[$product['order_goods_id']])) {
                throw new BaseException(['msg' => '提交的数据不合法']);
            }
            $item = $formData[$product['order_goods_id']];
            !empty($item['content']) && $data[$product['order_goods_id']] = [
                ...
                'order_goods_id' => $item['order_product_id'],
                'app_id' => self::$app_id,
                'purveyor_id' => $product['orderM']['purveyor_id']
            ];
        }
        return $data;
    }
    ...
    {
//        var_dump($commentList->toArray());die;
        // 生成评价图片数据
        $imageData = [];
        foreach ($commentList as $comment) {
