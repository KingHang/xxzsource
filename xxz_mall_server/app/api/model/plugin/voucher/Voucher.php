...
namespace app\api\model\plugin\voucher;

use app\common\model\plugin\voucher\Coupon as CouponModel;
...
class Voucher extends CouponModel
{
    ...
    {
        return $this->where('user_id', '=', $user_id)->column('voucher_id');
    }
    ...
    public function getList($user = false, $limit = null, $only_receive = false, $type = 0 , $shop_supplier_id = 0)
    {
        ...
        if ($type) {//显示平台优惠券
            $model = $model->where('purveyor_id', '=', 0);
        }
        ...
        if (!empty($couponList)) {
            foreach ($couponList as $key => $item) {
                ...
                $couponList[$key]['is_receive'] = in_array($item['voucher_id'], $userCouponIds);
            }
        }
        return $couponList;
    }
    ...
    public function getWaitList($data, $user = false, $type = 1, $status = 0)
    {
        ...
        // 构造查询条件
        if ($type) {
            $model = $model->where('purveyor_id', $data['shop_supplier_id']);
        } else {
...
//            $model = $model->where('coupon_id', 'not in', implode(',', $userCouponIds));
        ...
        if (!empty($couponList)) {
            foreach ($couponList as $item) {
                ...
                $item['is_get'] = in_array($item['voucher_id'], $userCouponIds);
            }
        }
        return $couponList;
    }
    ...
    {
        return $this->where('voucher_id', '=', $this['coupon_id'])->inc('receive_num')->update();
    }

    public function getWhereData($coupon_arr)
    {
        return $this->where('voucher_id', 'in', $coupon_arr)->select();
    }
    ...
    {
        return $this->where('voucher_id', 'in', $value)->select();
    }
}