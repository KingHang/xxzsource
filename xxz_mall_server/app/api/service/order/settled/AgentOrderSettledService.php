 ...
 * 分销商订单结算服务类
 */
class AgentOrderSettledService extends OrderSettledService
{
    /**
     * 构造函数
     */
        ...
        foreach ($this->supplierData as $supplier) {
            foreach ($supplier['productList'] as $product) {
                ...
                if (count($product['grade_ids']) > 0 && $product['grade_ids'][0] != '') {
                    if ($this->user && !in_array($this->user['grade_id'], $product['grade_ids'])) {
                        $this->error = '很抱歉，此商品仅特定会员可购买';
                        return false;
                    }
                }
                // 判断是否超过限购数量
                if ($this->user && $product['limit_num'] > 0) {
