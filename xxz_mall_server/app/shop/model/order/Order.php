...
use app\job\model\plus\agent\Month;
use app\shop\controller\plugin\agent\Venturebonus;
...
class Order extends OrderModel
{
    ...
    public function getList($dataType, $data = null)
    {
        ...
        // 获取数据列表
        return $model->with(['product' => ['image', 'refund'], 'user', 'supplier'])
            ->alias('order')
            ->field('order.*')
    ...
    private function setWhere($model, $data)
    {
            ...
            switch ($data['fields']) {
                ...
                case '7':
                    $goodsModel = (new OrderProductModel())->where('product_name', 'like', '%' . trim($data['search']) . '%')
                        ...
                        ->field('order_id')
                        ->buildSql();
                    $model = $model->join([$goodsModel => 'product'], 'product.order_id = order.order_id');
            }
        }
        //搜索自提门店
        ...
        if (isset($data['by']) && $data['by'] != '') {
            $order_ids = (new Month())->getOrderId($data['user_id'],$data['by']);
            $model = $model->where('order.order_id', 'in', $order_ids);
        }
        //搜索订单来源
        ...
        try {
            if ($orderInfo['product']) {
                foreach ($orderInfo['product'] as $product) {
                    ...
                    // 需要发货的
                    if (in_array($product['order_product_id'], $data['order_product_ids'])) {
                        $sendNum += 1;
                        $sendData[] = [
