...
use app\common\model\plugin\sign\Sign;
...
class UserService
{
    ...
    public function getData()
    {
        ...
        $data = [
            ...
            // 成交会员数
            'sign_user' => [
                'today' => number_format($this->getUserSignData($today)),
                'yesterday' => number_format($this->getUserSignData($yesterday))
            ],
        ];
        return $data;
    }
     ...
     * 通过时间段查询用户签到数
     */
    public function getSignUserByDate($days)
    {
        $data = [];
        foreach ($days as $day) {
            $data[] = [
                'day' => $day,
                'total_num' => $this->getUserSignData($day)
            ];
        }
        return $data;
    }
     ...
     * 获取用户签到
     */
    private function getUserSignData($startDate = null, $endDate = null)
    {
